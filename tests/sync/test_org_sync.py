...
def test_sync_org_incremental_skips_unchanged_repo_without_touching_dst():
    ...
    assert consumer.state_updates == {"repo1": "abc123"}


def test_sync_org_collects_state_updates_for_mirrored_and_skipped_not_failed():
    ctx = _ctx()
    ctx.src.default_branch.return_value = "main"
    ...
    assert consumer.state_updates == {"ok": "sha-ok"}
